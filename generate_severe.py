...
##################################################
#ROMOVE OLD IMAGES
os.system('rm -f /home/apache/climate/gensini/synthetic/F*.png')
##DEFINE GLOBAL PATHS#############################
...
for dt in dates: #begin loop for dates in dates list
	...
	#hail_marks = plt.plot(hail_FMx,hail_FMy,marker='+',color='green',alpha=.8,markersize=9, zorder=3)
	
	tor_report = np.array((maxUH_values>=200) & (maxUH_values <988),dtype=int)
	tor_count = np.add(tor_count,tor_report)
	tor_hrly_report_value = tor_report.sum()
	#print hrly_report_value
	...
	#tor_marks = plt.plot(tor_FMx,tor_FMy,marker='+',color='red',alpha=.8,markersize=9, zorder=3)
	
	wind_report = np.array((windgust_values>=27) & (max_reflectivity >=35) & (max_reflectivity < 122),dtype=int)
	wind_count = np.add(wind_count,wind_report)
	wind_hrly_report_value = wind_report.sum()
	#print hrly_report_value
	...
	#wind_marks = plt.plot(wind_FMx,wind_FMy,marker='+',color='blue',alpha=.8,markersize=9, zorder=3)
	
	wind_repo_lat = np.extract(wind_report == 1,lats)
	wind_repo_lon = np.extract(wind_report == 1,lons)
	hail_repo_lat = np.extract(hail_report == 1,lats)
	hail_repo_lon = np.extract(hail_report == 1,lons)
	tor_repo_lat = np.extract(tor_report == 1,lats)
	tor_repo_lon = np.extract(tor_report == 1,lons)
	...
	for latty,lonny in zip(wind_repo_lat,wind_repo_lon):
		CWAs = check(lonny,latty)
		if CWAs == None:
			pass
		else:
			windCWAlist.append(CWAs)
			CWAlist.append(CWAs)
	for latty,lonny in zip(tor_repo_lat,tor_repo_lon):
		CWAs = check(lonny,latty)
		if CWAs == None:
			pass
		else:
			torCWAlist.append(CWAs)
			CWAlist.append(CWAs)
	for latty,lonny in zip(hail_repo_lat,hail_repo_lon):
		CWAs = check(lonny,latty)
		if CWAs == None:
			pass
		else:
			hailCWAlist.append(CWAs)
			CWAlist.append(CWAs)
	final_CWA = itemfreq(CWAlist)
	tor_final_CWA = itemfreq(torCWAlist)
	wind_final_CWA = itemfreq(windCWAlist)
	hail_final_CWA = itemfreq(hailCWAlist)
	#print final_CWA
	...
	if len(final_CWA) == 0:
		bbox_props = dict(boxstyle="round", fc="k", ec="0.2", alpha=0.99)
	...
	#MAKE MESH MAP
	refc_map = m.pcolormesh(x,y,reflectivity,shading='flat',vmin=0,vmax=70,cmap=cmap,zorder=1)
	#MAKE COLORBAR
	colbar = m.colorbar(refc_map,"bottom",size="2%",ticks=intervals)
	#MAKE COLORBAR LABEL
	colbar.set_label('Simulated Radar Reflectivity Factor (dBZ)',fontsize=12)
	#MAKE TITLE
	plt.title(hr+'Z HRRR 1KM Reflectivity valid %s/%s/%s %sZ' % (MM,DD,YY,HH), fontsize=16)
	#plt.annotate('Total Gridpoints: '+str(tor_cumulative_report_value),xycoords='figure pixels',xy=(999,25),color=('purple'), size=10)
	#plt.annotate('Hourly Gridpoints: '+str(tor_hrly_report_value),xycoords='figure pixels',xy=(788,25),color=('purple'), size=10)
	plt.annotate('Victor Gensini @gensiniwx',xycoords='figure pixels',xy=(122,25), color=('darkred'),size=10)
	plt.subplots_adjust(left=None,right=None,top=None,bottom=None,wspace=None,hspace=None)
	
	#SAVE FIGURE
	plt.savefig(out_path+filename_str + 'rad.png',dpi=100,transparent=True)
	plt.clf()
	...
	#plt.annotate('Victor Gensini @gensiniwx',xycoords='figure pixels',xy=(122,25), color=('darkred'),size=10)
	plt.subplots_adjust(left=None,right=None,top=None,bottom=None,wspace=None,hspace=None)
	hail_marks = m.plot(hail_FMx,hail_FMy,marker='+',color='green',alpha=.8,markersize=9, zorder=3)
	#SAVE FIGURE
	plt.savefig(out_path+filename_str + 'hail.png',dpi=100,transparent=True)
	plt.clf()
	...
	plt.annotate('Victor Gensini @gensiniwx',xycoords='figure pixels',xy=(122,25), color=('darkred'),size=10)
	plt.subplots_adjust(left=None,right=None,top=None,bottom=None,wspace=None,hspace=None)
	tor_marks = m.plot(tor_FMx,tor_FMy,marker='+',color='red',alpha=.8,markersize=9, zorder=3)
	#SAVE FIGURE
	plt.savefig(out_path+filename_str + 'tor.png',dpi=100,transparent=True)
	plt.clf()
	...
	plt.annotate('Victor Gensini @gensiniwx',xycoords='figure pixels',xy=(122,25), color=('darkred'),size=10)
	plt.subplots_adjust(left=None,right=None,top=None,bottom=None,wspace=None,hspace=None)
	wind_marks = m.plot(wind_FMx,wind_FMy,marker='+',color='blue',alpha=.8,markersize=9, zorder=3)
	#SAVE FIGURE
	plt.savefig(out_path+filename_str + 'wind.png',dpi=100,transparent=True)
	plt.clf()
